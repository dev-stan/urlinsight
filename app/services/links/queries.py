...
def get_daily_clicks(db: Session, link_id: int):
    return (
        db.query(func.date(ClickEvent.created_at, type_=Date).label("date"), func.count(ClickEvent.id).label("clicks"))
...
def get_daily_unique_visits(db: Session, link_id: int):
    return (
        db.query(UniqueVisit.date, func.count(UniqueVisit.id).label("unique_visitors"))
        .filter(UniqueVisit.link_id == link_id)
        .group_by(UniqueVisit.date)
        .all()
    )
